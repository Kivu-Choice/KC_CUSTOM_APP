...
def generate_quarterly_ldcs():
    ...
    `custom_job_level` is Manager, Director, or Chief for the current quarter.
    Trigged by cron on 15th of Mar/Jun/Sep/Dec. (temporarily set to 24th for testing)
    """
    ...
    # Fetch active employees matching specific job levels
    employees = frappe.get_all(
        "Employee",
        filters={
            "status": "Active",
            "custom_job_level": ["in", ["Director", "Manager", "Chief"]],
            "reports_to": ["is", "set"],  # <--- Excludes employees where reports_to is empty
        },
...
def create_ldc_for_employee(employee: str, department: str, reports_to: str, quarter_label: str, fiscal_year: str):
    if not reports_to:
        frappe.log_error(f"Cannot create LDC for {employee}: Missing 'reports_to' manager.", "create_ldc_for_employee")
        return
